...
import React__default, { useMemo, useEffect, useLayoutEffect, useContext, useReducer, useRef, createElement, Component } from 'react';
...
var TestComponent = function (_a) {
    ...
    return (React__default.createElement("div", { className: "test-component test-component-" + theme },
        ...
        React__default.createElement("h2", null, "Made with love by Harvey")));
};
...
var exampleVideoSession = {
    id: "001",
    name: "Example videoSession"
};

var _a;
// load in one sample video
var initialState = (_a = {},
    _a[exampleVideoSession.id] = exampleVideoSession,
    _a);
var videoSessionsSlice = createSlice({
    name: 'videoSessions',
    ...
    reducers: {
        ...
        addVideoSessions: function (state, action) {
            action.payload.forEach(function (videoSession) {
                state[videoSession.id] = videoSession;
            });
        }
    }
});
var addVideoSessions = videoSessionsSlice.actions.addVideoSessions;
var videoSessionsReducer = videoSessionsSlice.reducer;
...
var reactIs_1 = reactIs.isValidElementType, isContextConsumer;
...
var useIsomorphicLayoutEffect = isHopefullyDomEnvironment ? useLayoutEffect : useEffect;
...
_ref) {
  ...
  return function wrapWithConnect(WrappedComponent) {
    if (process.env.NODE_ENV !== 'production') {
      browser(reactIs_1(WrappedComponent), "You must pass a component to the function returned by " + (methodName + ". Instead received " + stringifyComponent(WrappedComponent)));
    }
    ...
    function ConnectFunction(props) {
      ...
      var ContextToUse = useMemo(function () {
        ...
        return propsContext && propsContext.Consumer && isContextConsumer() ? propsContext : Context;
      ...
      browser(didStoreComeFromProps || didStoreComeFromContext, "Could not find \"store\" in the context of " + ("\"" + displayName + "\". Either wrap the root component in a <Provider>, ") + "or pass a custom React context provider to <Provider> and the corresponding " + ("React context consumer to " + displayName + " in connect options.")); // Based on the previous check, one of these must be true
...
var withConnectedCounter = function (BaseComponent) {
    ...
    var Hoc = /** @class */ (function (_super) {
        ...
        Hoc.prototype.render = function () {
            var _a = this.props, count = _a.count, onIncrement = _a.onIncrement, overrideCount = _a.overrideCount, restProps = __rest(_a, ["count", "onIncrement", "overrideCount"]);
            return (createElement(BaseComponent, __assign({ count: overrideCount || count, onIncrement: onIncrement }, restProps)));
        };
        // Enhance component name for debugging and React-Dev-Tools
        Hoc.displayName = "withConnectedCount(" + BaseComponent.name + ")";
        // reference to original wrapped component
        Hoc.WrappedComponent = BaseComponent;
        return Hoc;
    }(Component));
    var ConnectedHoc = connect(mapStateToProps, dispatchProps)(Hoc);
    return ConnectedHoc;
};
...
export { CounterDisplay, TestComponent, addVideoSessions, increment, rootReducer, store, withConnectedCounter };
